...
fuzz_mutator!(|data: &mut [u8], size: usize, max_size: usize, seed: u32| {
    ...
    // we construct a deterministic seed for our own rng
    let seed = seed as u128;
    let seed = (seed << 96) | (seed << 64) | (seed << 32) | seed;
    // create rng for invoking the grammar generator
    let mut rng = rand_pcg::Pcg64Mcg::new(seed);

    let gen_new = data.len() < parser_config_bytes.len() || rng.gen_bool(GRAMMAR_NEW_PROB);
    if gen_new {
        ...
        // mutate the bytes for the parser config
        let cl = parser_config_bytes.len() - 1;
        libfuzzer_sys::fuzzer_mutate(&mut parser_config_bytes[1..], cl, cl);
        ...
        // because the testcase is prefixed by an arbitrary HttparserConfig, we skip mutating the
        // first byte,
        libfuzzer_sys::fuzzer_mutate(&mut data[1..], size - 1, max_size - 1)
    }
});
...
fn parse(config: Option<HttparserConfig>, data: &[u8]) -> bool {
    ...
    match config {
        Some(fuzzed_config) => httparse::ParserConfig::default()
            ...
            .parse_request(&mut req, data)
            .is_ok(),
        None => req.parse(data).is_ok(),
    }
}
